...
# --- 9b. Ensure .claude/ is gitignored (auto-generated by start.sh, never committed) ---
if [[ -f "$PROJECT_DIR/.gitignore" ]]; then
    grep -qF '.claude/' "$PROJECT_DIR/.gitignore" 2>/dev/null || \
        echo '.claude/' >> "$PROJECT_DIR/.gitignore"
else
    echo '.claude/' > "$PROJECT_DIR/.gitignore"
fi
echo "Ensured .claude/ is in .gitignore"
