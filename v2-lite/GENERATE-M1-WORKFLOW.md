...
and save. The per-task engine **reuses the proven v2-lite loop** (`nightcrawler-loop`) — M1 only
...
- **Human preflight before launching:** run `bash install.sh --check` yourself (gate in sync). The
  workflow runs from the *target* repo and can't locate the Nightcrawler source checkout, so gate-freshness
...
> ultracode: Build a dynamic workflow `nightcrawler-feat` that runs an ordered list of tasks as
> ONE feature, reusing the existing `nightcrawler-loop` per-task logic. `args` = `{ feat: "<title>",
...
>   `add-divide-helper-1pkgi21`. Used everywhere — branch, state file, idSalt. Feat branch: `nc/feat-<featId>`
>   (hyphen) cut from the current base branch. (Slugify truncates ~24 chars so names stay clean.)
> - Each task: passing `branchPrefix:"nc/feat/<featId>/"` + `idSalt:featId` to the v2-lite loop yields branch
>   `nc/feat/<featId>/<slug>-<id>` where `id = FNV-1a(featId + taskSpec)`. That suffix `<slug>-<id>` IS the
...
> - Persistent state file OUTSIDE any worktree: `~/.claude/nightcrawler/feats/<featId>.json`
...
>    workflow can't locate the Nightcrawler source.)
...
>    `nightcrawler-loop` workflow uses (classify→route model, thin Codex reviewer, gated on P0/P1/P2,
>    deterministic verify). **Pass feat-scoped identity into that loop** so its branch is feat-namespaced,
>    not derived from task text alone: `branchPrefix: "nc/feat/<featId>/"` and `idSalt: "<featId>"`
>    (nightcrawler-loop now accepts these args; the task branch becomes `nc/feat/<featId>/<slug>-<id>`).
...
>    `~/.claude/nightcrawler/reports/<featId>.json` (out-of-tree). Leave the feat branch for the human to merge.
>
...
- [ ] Task branches are **feat-scoped** via `branchPrefix`/`idSalt` passed to nightcrawler-loop (NOT its
      default task-text-only naming) → `nc/feat/<featId>/<slug>-<id>`.
- [ ] State file is under `~/.claude/nightcrawler/feats/` (per-user, OUTSIDE the worktree).
...
If all pass: `/workflows` → save as `/nightcrawler-feat`.
