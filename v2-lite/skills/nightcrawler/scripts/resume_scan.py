...
the `nightcrawler-feat` workflow to pick the run back up.
...
WIRING TO A SCHEDULE (the scanner only REPORTS — the schedule does the re-invoke):
  ...
  2. For each object in the printed JSON array, re-invoke the feat workflow with it VERBATIM:
         workflow('nightcrawler-feat', <object>)
...
SAFE-to-auto-resume rule (ALL must hold):
  ...
  - the state file has been idle >= NC_RESUME_STALE_SEC (default 1800s) — a still-LIVE run touches
...
def stale_seconds():
    try:
        v = int(os.environ.get("NC_RESUME_STALE_SEC", "1800"))
